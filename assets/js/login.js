    ...
    var_form.submit((e) => {
      ...
      if (lockout && new Date().getTime() < lockout) {
        ...
        Swal.fire({
          ...
          html: `Tente novamente em ${Math.ceil(remainingTime)} segundos.`,
          icon: 'error',
          timer: 2000,
        ...
        success: function (response) {
          ...
          if (response.status == 1) {
            ...
            if (attempts >= maxAttempts) {
              const lockoutExpires = new Date().getTime() + lockoutTime;
              localStorage.setItem('lockout', lockoutExpires);
              Swal.fire({
                title: 'Você está bloqueado!',
                html: `Tente novamente em ${lockoutTime / 1000} segundos.`,
                icon: 'error',
                timer: 2000,
                timerProgressBar: true,
              });
